...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="outputs",
        help="Folder to analyze; results are written into <run_dir>/analysis",
    )
...
def main() -> None:
    ...
    if os.path.isdir(run_arg):
        ...
        if os.path.isdir(candidate):
            ...
            if len(matches) == 1:
                ...
                # Fallback: treat the argument as a path that doesn't exist -> fail with informative message
                raise SystemExit(
                    f"No optimizer_history.npz files found under: {args.run_dir}\nTried: {run_arg} and outputs/{run_arg}"
                )
    ...
    if not files:
        raise SystemExit(f"No optimizer_history.npz files found under: {run_root}")
