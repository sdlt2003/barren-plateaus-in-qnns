...
from qiskit_algorithms.optimizers import COBYLA, QNSPSA, NFT
...
from helpers.utils_runtime import (  # noqa: E402
    describe_runtime_mode,
    normalize_execution_mode,
    normalize_runtime_max_time,
    runtime_execution_mode,
)
from helpers.architectures import available_architectures, build_architecture, parse_depth_split  # noqa: E402
...
def run_qubit_point(
    *,
    ...
    history, counts = init_trackers("cobyla", "qnspsa", "nft")
    run_status = {
        "status": "failed",
    ...
    try:
        ...
        # NFT is opt-in on hardware (not part of "both") to avoid changing the
        # cost profile of the existing cobyla/qnspsa grids.
        if optimizer == "nft":
            objective_nft = make_objective(
                "nft",
                counts=counts,
                history=history,
                estimator=estimator,
            ...
            run_optimizer(
                "nft",
                NFT(maxiter=resolved_budget_evals),
                objective_nft,
                initial_parameters,
                counts=counts,
            )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="both",
        choices=["both", "cobyla", "qnspsa", "nft"],
        help="Which optimizer(s) to run. Default: both (cobyla+qnspsa). 'nft' is opt-in.",
    )
