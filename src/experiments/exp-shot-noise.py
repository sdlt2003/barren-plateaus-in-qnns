...
def optimizer_compare(
    ...
    if track_grad_norm or track_grad_var:
        gradient = build_simulation_gradient(
            "shot-noise",
            estimator=estimator,
    ...
    if training_mode == "layerwise":
        ...
        print(
            ...
            f"training_mode=layerwise | inner={layerwise_inner} | shots~{num_shots}{grad_note}"
        )
        ...
        print(
            f"Architecture={architecture} | Budget mode: {budget_mode} | "
            f"num_params={num_params} | k={budget_k} | budget_evals={budget_evals} | "
            f"optimizers={list(optimizers)} | shots~{num_shots}{grad_note}"
        )
        history, counts = init_trackers(*optimizers)
        print(f"Running optimizers with Monte Carlo noise (approx. {num_shots} shots)...")
        run_optimizer_suite(
            optimizers,
            estimator=estimator,
            ansatz=ansatz_decomposed,
