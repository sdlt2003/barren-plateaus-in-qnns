...
def analyze_single_run(run_root: str) -> int:
    ...
    # Merge split-optimizer layouts (e.g. qubits_32/opt_cobyla and opt_qnspsa, or
    # per-optimizer subfolders for any of N optimizers) back into a single row per
    # (seed, qubits) so downstream CSVs and plots remain comparable.
    ...
    for row in rows:
        ...
        if key not in merged:
            merged[key] = RunSummary(
                seed=row.seed,
                qubits=row.qubits,
                final_cost=dict(row.final_cost),
                evals=dict(row.evals),
            )
            continue
        base = merged[key]
        for name, cost in row.final_cost.items():
            if base.final_cost.get(name) is None and cost is not None:
                base.final_cost[name] = cost
        for name, n_evals in row.evals.items():
            if base.evals.get(name) is None and n_evals is not None:
                base.evals[name] = n_evals
